...
const empresa = ref(props.empresa);
const membresias = ref();

const form = useForm({
    membresia_id: empresa.value.membresia_id,
...
const submit = async () => {
    try {
        form.post(route("empresas.update", empresa.value.id), {
            onFinish: () => {
    ...
    <Layout :title="title">
        ...
        <div class="overflow-hidden sm:rounded-lg">
            ...
            <div class="breadcrumbs flex">
                ...
                <Link :href="route('empresas.edit', empresa.id)" class="active">
                    <h3>Editar</h3>
                </Link>
            </div>
        </div>
