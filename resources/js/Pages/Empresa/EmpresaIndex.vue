<script setup>
import { Head, Link, usePage } from "@inertiajs/vue3";
import { ref, onMounted, watch } from "vue";
import axios from "axios";
import Layout from "@/Layouts/Layout.vue";
import PrimaryButton from "@/Components/PrimaryButton.vue";
import { confirmDialog, showToast } from "../utils/SweetAlert.service";
import DataTable from "primevue/datatable";
import Column from "primevue/column";
import InputText from "primevue/inputtext";

// Definir propiedades
const props = defineProps({});

const totalRecords = ref(0);
const rows = ref(10);
const first = ref(0);
const empresas = ref([]);
const globalFilter = ref("");
const filters = ref({});
const sortField = ref("id"); // Valor predeterminado
const sortOrder = ref(1);
const title = "Pilares";
const userPremissions = usePage().props.auth.user.permissions;

// Función para obtener áreas
async function getEmpresas(
    page = 1,
    rowsPerPage = rows.value,
    filter = "",
    sortField = "id",
    sortOrder = 1
) {
    try {
        const response = await axios.get("/api/empresas", {
            params: {
                page,
                rows: rowsPerPage,
                filter,
                sortField,
                sortOrder: sortOrder === 1 ? "asc" : "desc",
            },
        });
        empresas.value = response.data.data;
        totalRecords.value = response.total;
        first.value = (response.current_page - 1) * rows.value;
    } catch (error) {
        console.error(error);
    }
}

// Manejar paginación
const onPage = (event) => {
    const page = event.page + 1;
    rows.value = event.rows; // Actualizar filas por página
    getEmpresas(
        page,
        rows.value,
        globalFilter.value,
        sortField.value,
        sortOrder.value
    );
};

// Manejar ordenación
const onSort = (event) => {
    sortField.value = event.sortField || "id";
    sortOrder.value = event.sortOrder;
    getEmpresas(
        1,
        rows.value,
        globalFilter.value,
        sortField.value,
        sortOrder.value
    );
};

// Obtener áreas al montar el componente
onMounted(() => {
    getEmpresas();
});

// Eliminar área
const deleteEmpresa = async (id) => {
    try {
        const result = await confirmDialog(
            "Estas seguro?",
            "Ya no podras revertir esto!",
            "warning"
        );
        if (result.isConfirmed) {
            await axios.delete(route("empresas.destroy", id));
            empresas.value = areas.value.filter((empresa) => empresa.id !== id);
            showToast("El registro ha sido eliminado", "success");
        }
    } catch (error) {
        console.error(error);
    }
};

// Actualizar filtro global
watch(globalFilter, (newValue) => {
    filters.value = {
        global: { value: newValue, matchMode: "contains" },
    };
    getEmpresas(1, rows.value, newValue, sortField.value, sortOrder.value);
});
</script>

<style scoped>
.mb-3 {
    margin-bottom: 1rem;
}
</style>

<template>
    <Layout :titulo="title">
        <Head title="Compañias" />
        <div class="overflow-hidden sm:rounded-lg">
            <div class="breadcrumbsTitulo px-1">
                <h3>Compañias</h3>
            </div>
            <div class="breadcrumbs flex">
                <Link :href="route('dashboard')" class="px-1">
                    <h3>Home -</h3>
                </Link>
                <Link class="active" href="#">
                    <h3>Compañias</h3>
                </Link>
            </div>
        </div>

        <div class="py-2">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div
                    class="px-4 py-2 flex justify-end bg-white border-b border-gray-200"
                >
                    <div v-if="userPremissions.includes('pilares_crear')">
                        <PrimaryButton
                            :href="route('empresas.create')"
                            class="m-4 pi pi-plus"
                        >
                            <span
                                class="p-1"
                                :style="{
                                    fontSize: '10px',
                                }"
                            >
                                Nueva compañia</span
                            >
                        </PrimaryButton>
                    </div>
                </div>

                <div class="px-4 py-2 bg-white border-b border-gray-200">
                    <div class="container mx-auto">
                        <InputText
                            v-model="globalFilter"
                            placeholder="Buscar..."
                            class="mb-3"
                        />

                        <DataTable
                            :value="empresas"
                            paginator
                            :rows="rows"
                            :totalRecords="totalRecords"
                            :lazy="true"
                            :first="first"
                            @page="onPage"
                            @sort="onSort"
                            :rowsPerPageOptions="[5, 10, 20, 50]"
                            tableStyle="min-width: 50rem"
                            :filters="filters"
                            :globalFilterFields="[
                                'id',
                                'nombre',
                                'descripcion',
                            ]"
                            :sortField="sortField"
                            :sortOrder="sortOrder"
                            class="p-datatable-sm p-datatable-striped p-datatable-gridlines"
                        >
                            <template #empty> No data found. </template>
                            <Column
                                field="id"
                                header="ID"
                                headerStyle="width:4em;"
                                bodyStyle="text-align:center;"
                                sortable
                            ></Column>
                            <Column
                                field="nombre"
                                header="Nombre"
                                headerStyle="width:4em;"
                                bodyStyle="text-align:center;"
                                bodyClass="text-center"
                                sortable
                            ></Column>
                            <Column
                                field="membresia.nombre"
                                header="Plan"
                                headerStyle="width:4em;"
                                bodyClass="text-center"
                                sortable
                            ></Column>
                            <Column
                                field="pais"
                                header="Pais"
                                headerStyle="width:4em;"
                                bodyClass="text-center"
                                sortable
                            ></Column>
                            <Column
                                field="ciudad"
                                header="Ciudad"
                                headerStyle="width:4em;"
                                bodyClass="text-center"
                                sortable
                            ></Column>
                            <Column
                                field="estado"
                                header="Estado"
                                headerStyle="width:4em;"
                                bodyClass="text-center"
                                sortable
                            ></Column>
                            <Column
                                field="sitio_web"
                                header="Sitio web"
                                headerStyle="width:4em;"
                                bodyClass="text-center"
                                sortable
                            >
                                <template #body="slotProps">
                                    <a
                                        class="text-blue-500 hover:text-blue-700"
                                        :href="slotProps.data.sitio_web"
                                        target="_blank"
                                        >{{ slotProps.data.sitio_web }}</a
                                    >
                                </template>
                            </Column>

                            <Column header="" headerStyle="width:4em;">
                                <template #body="slotProps">
                                    <div class="text-center">
                                        <div class="flex justify-evenly">
                                            <div
                                                v-if="
                                                    userPremissions.includes(
                                                        'pilares_editar'
                                                    )
                                                "
                                                class="inline"
                                            >
                                                <PrimaryButton
                                                    class="pi pi-file-edit me-2"
                                                    :href="
                                                        route(
                                                            'empresas.edit',
                                                            slotProps.data.id
                                                        )
                                                    "
                                                >
                                                    <span
                                                        class="p-1"
                                                        :style="{
                                                            fontSize: '10px',
                                                        }"
                                                    >
                                                        Editar pilar</span
                                                    >
                                                </PrimaryButton>
                                            </div>

                                            <div
                                                v-if="
                                                    userPremissions.includes(
                                                        'pilares_eliminar'
                                                    )
                                                "
                                                class="inline"
                                            >
                                                <!-- <PrimaryButton
                                                        class="pi pi-trash me-2"
                                                        @click.prevent="
                                                            deleteEmpresa(
                                                                slotProps.data
                                                                    .id
                                                            )
                                                        "
                                                    >
                                                    </PrimaryButton> -->
                                            </div>
                                        </div>
                                    </div>
                                </template>
                            </Column>
                        </DataTable>
                    </div>
                </div>
            </div>
        </div>
    </Layout>
</template>
